...
export { waitForUrl };
export async function startBackendProcess(isDev: boolean): Promise<ChildProcess | null> {
  ...
  if (isDev) {
    ...
    return spawn(getPnpmCommand(), ['--dir', backendRoot, 'dev'], {
      ...
      env: {
        ...
        PORT: String(port),
        DATABASE_URL:
          process.env.DATABASE_URL ?? path.join(app.getPath('userData'), 'ai-workflow.sqlite'),
      },
      detached: false,
    });
  }
