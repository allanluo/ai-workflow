...
export function parseShotPlanForEdit(asset: Asset): ParsedShotPlanForEdit | null {
...
export type LocatedShot = {
  shot: ShotPlanItem;
  shotPath: string; // JSON Pointer to the shot object (without escaping)
  sceneTitle?: string;
};
...
export function updateShotImageOverrideInPlan(
  ...
  // Mutates the plan object (callers should clone first).
  ...
  if (sceneMatch) {
    ...
    const scene = scenes[sceneIndex] ?? {};
    const shots = (scene.shots as unknown[]) as ShotPlanItem[];
